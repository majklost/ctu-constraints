...
class SpatialTransformer(nn.Module):
    ...
    def forward(
        self, moving_image: torch.Tensor, deformation_field: torch.Tensor
    ) -> torch.Tensor:
        """
...
class IntegrateVelocityField(nn.Module):
    ...
    def __init__(
        ...
        self.transformer = SpatialTransformer(
            interpolation_mode=interpolation_mode, align_corners=align_corners
        )
    ...
    def forward(self, velocity_field: torch.Tensor) -> torch.Tensor:
        ...
        for _ in range(self.steps):
            ...
            velocity_field = velocity_field + self.transformer(
...
class ResizeDisplacementField(nn.Module):
    ...
    def __init__(
        self,
        scale_factor: float | int | None = 1.0,
        interpolation_mode: str = "linear",
        align_corners: bool = True,
    ) -> None:
        """
        ...
        Parameters
        ----------
        scale_factor : float | int | None, optional
